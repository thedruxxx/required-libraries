    ...
    packetProcessors = utils.switch({
        [packet.PACKETS_ID.START_TRANSFER] = function()
            ...
            if sessions[sessionId] ~= nil then
                ...
                logger.debug(('session collision: session %d was removed'):format(sessionId))
                return
            end
        ...
        [packet.PACKETS_ID.DATA] = function()
            ...
            if sess ~= nil then
                ...
                logger.debug('cannot found session for data packet: ' .. sessionId)
                logger.debug('sessions list:\n  ' .. inspect(sessions))
        ...
        [packet.PACKETS_ID.HANDLER_ID] = function()
            ...
            if sess ~= nil then
                ...
                logger.debug('cannot found session for handler id packet: ' .. sessionId)
                logger.debug('sessions list:\n  ' .. inspect(sessions))
            end
        end,
        [packet.PACKETS_ID.STOP_TRANSFER] = function()
            ...
            if sess ~= nil then
                ...
                logger.debug('cannot found session for stop transfer packet: ' .. sessionId)
                logger.debug('sessions list:\n  ' .. inspect(sessions))
            end
        end,
        default = function() logger.debug('cannot identify packet') end
    })
...
function proto.sendData(...)
    logger.debug('sendData function is deprecated and will be removed, use packData instead')
